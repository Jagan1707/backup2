...
//const sendnotification =require('../utill/notifi.json')
 ...
 router.post('/addItems',isAdmin,async(req,res)=>{
    try{
...
router.get('/showAllitems',authVerify,async(req,res)=>{
    try{
        await elecSchema.find().exec().then(data=>{
            res.json({status:'success',message:'show All items',"result":data})
        })
    }catch(err){
        res.json({'error':err.message});
    }
...
router.get('/indiviual',async(req,res)=>{
    try{
        const uuid = req.query.uuid
        await elecSchema.findOne({uuid:uuid}).exec().then(data=>{
            res.json({"status":"successfull","result":data})
        }).catch(err=>{
...
//find category bassed product
router.get('/getAllItems',async(req,res)=>{
    try{
        const uuid = req.query.uuid
        await elecSchema.find({categoryUuid:uuid}).exec().then(data=>{
